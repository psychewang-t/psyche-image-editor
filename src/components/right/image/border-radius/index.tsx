...
const BorderRadius = () => {
  ...
  const init = () => {
    ...
    if (currentObj.clipPath?.type === 'rect' && !isNaN(Number(currentObj.clipPath.fill))) {
      setBorderRadius(Math.round(Number((currentObj.clipPath as fabric.Rect).fill)));
    } else {
      setBorderRadius(0);
    }
  ...
  return (
    <div className="border-radius-wrapper">
      ...
      <div className="global-common-bg-color border-radius-box">
        <NodeRound theme="outline" size="16" fill={normalIconColor} strokeWidth={4} className="global-icon-hover" />
